...
import { Plus, Trash2, Edit, Copy, Download, Upload, FileCode, Terminal, Table2, Network, Puzzle, Code, Zap, Archive } from 'lucide-react';
...
type SubTab = 'overview' | 'query' | 'tables' | 'schema' | 'extensions' | 'functions' | 'triggers' | 'backup';
...
export default function DatabaseManager() {
  ...
  const subTabs = [
    { id: 'overview' as SubTab, label: 'Огляд', icon: Table2 },
  ...
  return (
    <div className="space-y-6">
      ...
      {activeSubTab === 'query' && <QueryExecutor />}
      {activeSubTab === 'tables' && <TableBrowser />}
      {activeSubTab === 'schema' && <SchemaVisualizer />}
      {activeSubTab === 'extensions' && <ExtensionManager />}
      {activeSubTab === 'functions' && <FunctionsManager />}
      {activeSubTab === 'triggers' && <TriggersRules />}
      {activeSubTab === 'backup' && <BackupRestore />}
